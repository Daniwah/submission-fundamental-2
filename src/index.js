...
async function addNoteToListAndStorage(title, content) {
  try {
    ...
    deleteButton.addEventListener('click', async function() {
      try {
        const deleteResponse = await fetch(`https://notes-api.dicoding.dev/v2/notes/${responseData.data.id}`, {
          method: 'DELETE'
        });
    ...
    archiveButton.addEventListener('click', async function() {
      try {
        ...
        if (noteItem.classList.contains('archived')) {
          archiveResponse = await fetch(`https://notes-api.dicoding.dev/v2/notes/${responseData.data.id}/unarchive`, {
            method: 'POST'
          });
        } else {
          archiveResponse = await fetch(`https://notes-api.dicoding.dev/v2/notes/${responseData.data.id}/archive`, {
            method: 'POST'
          });
        }
...
async function loadNonArchivedNotesFromAPI() {
  try {
    ...
    notesData.forEach(note => {
      addNoteToList(note.title, note.body, note.createdAt, note.archived, note.id);
    });
  } catch (error) {
...
function addNoteToList(title, content, createdAt, archived, id) {
  ...
  deleteButton.addEventListener('click', async function() {
    try {
      const deleteResponse = await fetch(`https://notes-api.dicoding.dev/v2/notes/${id}`, {
        method: 'DELETE'
      });
  ...
  archiveButton.addEventListener('click', async function() {
    try {
      const archiveResponse = await fetch(`https://notes-api.dicoding.dev/v2/notes/${id}/${archived ? 'unarchive' : 'archive'}`, {
        method: 'POST'
      });
